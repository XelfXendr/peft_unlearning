...
def download_model(hf_token: str) -> tuple[AutoModelForCausalLM, AutoTokenizer]:
    ...
    return AutoModelForCausalLM.from_pretrained('semeval25-unlearning-model'), AutoTokenizer.from_pretrained("allenai/OLMo-7B-0724-Instruct-hf")
...
def download_model_1B(hf_token: str) -> AutoModelForCausalLM:
    ...
    return AutoModelForCausalLM.from_pretrained('semeval25-unlearning-1B-model'), AutoTokenizer.from_pretrained("allenai/OLMo-1B-0724-hf")

def download_datasets(hf_token: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
